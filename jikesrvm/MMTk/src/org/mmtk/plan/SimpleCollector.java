	...
	public void collectionPhase(short phaseId, boolean primary) {
		...
		if (phaseId == Simple.SOFT_REFS) {
			if (primary) {
				if (Options.noReferenceTypes.getValue())
					...
					VM.softReferences.scan(getCurrentTrace(), global()
							.isCurrentGCNursery());
			}
			return;
		}
		...
		if (phaseId == Simple.WEAK_REFS) {
			if (primary) {
				if (Options.noReferenceTypes.getValue())
					...
					VM.weakReferences.scan(getCurrentTrace(), global()
							.isCurrentGCNursery());
			}
			return;
		}
		...
		if (phaseId == Simple.FINALIZABLE) {
			if (primary) {
				if (Options.noFinalizer.getValue())
					...
					VM.finalizableProcessor.scan(getCurrentTrace(), global()
							.isCurrentGCNursery());
			}
			return;
		}
		...
		if (phaseId == Simple.PHANTOM_REFS) {
			if (primary) {
				if (Options.noReferenceTypes.getValue())
					...
					VM.phantomReferences.scan(getCurrentTrace(), global()
							.isCurrentGCNursery());
			}
			return;
		}
		...
		if (phaseId == Simple.FORWARD_REFS) {
			if (primary && !Options.noReferenceTypes.getValue()
				...
				VM.softReferences.forward(getCurrentTrace(), global()
						.isCurrentGCNursery());
				VM.weakReferences.forward(getCurrentTrace(), global()
						.isCurrentGCNursery());
				VM.phantomReferences.forward(getCurrentTrace(), global()
						.isCurrentGCNursery());
			}
			return;
		}
		...
		if (phaseId == Simple.FORWARD_FINALIZABLE) {
			if (primary && !Options.noFinalizer.getValue()
					&& VM.activePlan.constraints().needsForwardAfterLiveness()) {
				VM.finalizableProcessor.forward(getCurrentTrace(), global()
						.isCurrentGCNursery());
			}
			return;
		}
		...
		if (Options.sanityCheck.getValue()
				&& sanityLocal.collectionPhase(phaseId, primary)) {
			return;
		}
